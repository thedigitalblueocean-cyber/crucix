...
Output ONLY valid JSON array. No markdown, no explanation, no code fences. Each object:
{
  ...
  try {
    ...
    if (ideas && ideas.length > 0) {
      console.log(`[LLM Ideas] Parsed ${ideas.length} ideas from ${result.model || provider.model}`);
      return ideas;
    }
    console.warn('[LLM Ideas] No valid ideas parsed from response');
    console.warn('[LLM Ideas] Raw response preview:', (result.text || '').substring(0, 300));
    return null;
  } catch (err) {
...
/**
 * Parse LLM response into ideas array. Handles markdown code blocks and nested objects.
 */
