 ...
 *
 * After deploy:
 *   The contract address is printed and auto-patched into your .env as:
...
if (!PRIVATE_KEY) {
  ...
  console.error('   Add it as: ANCHOR_PRIVATE_KEY=0x<your-private-key>\n');
  process.exit(1);
}

// ---- Pre-compiled ABI + Bytecode ----
// Compiled from contracts/CVS512Anchor.sol with solc 0.8.20, optimizer 200 runs.
// Re-compile only if you modify the contract: npx solc@0.8.20 --abi --bin --optimize --output-dir /tmp/out contracts/CVS512Anchor.sol
...
console.log('[deploy] Bytecode: pre-compiled (solc 0.8.20, optimizer 200 runs)');
...
try {
  ...
  console.error(`\n❌ Cannot connect to RPC: ${err.message}`);
  console.error(`   Check ANCHOR_RPC_URL and your internet connection.\n`);
  process.exit(1);
}
